...
function buildSummarySection(summary: CommentSummary): string {
  ...
  const lines = [
    ...
    `🖼️ **${summary.totalFrames}** frame${summary.totalFrames !== 1 ? 's' : ''} detected`,
  ];

  if (summary.successfulRenders > 0) {
    lines.push(`✅ **${summary.successfulRenders}** rendered successfully`);
  }

  if (summary.failedRenders > 0) {
    lines.push(`❌ **${summary.failedRenders}** failed to render`);
  }
...
function buildFooter(commitSha: string): string {
  ...
  return `_Generated by [Pencil Actions](https://github.com/remotestate/pencil-actions) for commit ${shortSha}_`;
}
